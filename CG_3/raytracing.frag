...
void InitializeDefaultScene() {
    ...
    spheres[0].Center = vec3(-2.5, -2.5, -1.0);
    spheres[0].Radius = 1.0;
    spheres[0].MaterialIdx = 6;

    spheres[1].Center = vec3(0.0, 0.0, -1.0);
    spheres[1].Radius = 1.0;
    spheres[1].MaterialIdx = 6;

    spheres[2].Center = vec3(2.5, 2.5, -1.0);
    ...
    /** ЗЕРКАЛЬНЫЙ КУБ В ЛЕВОМ ВЕРХНЕМ УГЛУ **/
    // Определение координат для куба размером 2x2x2
    vec3 A = vec3(-4.5, 4.0, 3.5);  // левый верхний ближний угол
    vec3 B = vec3(-2.5, 4.0, 3.5);  // правый верхний ближний угол
    vec3 C = vec3(-2.5, 2.0, 3.5);  // правый нижний ближний угол
    vec3 D = vec3(-4.5, 2.0, 3.5);  // левый нижний ближний угол
    vec3 E = vec3(-4.5, 4.0, 1.5);  // левый верхний дальний угол
    vec3 F = vec3(-2.5, 4.0, 1.5);  // правый верхний дальний угол
    vec3 G = vec3(-2.5, 2.0, 1.5);  // правый нижний дальний угол
    vec3 H = vec3(-4.5, 2.0, 1.5);  // левый нижний дальний угол
...
void InitializeDefaultLightMaterials() {
    ...
    materials[6].Color = vec3(0.8, 0.8, 1.0); // Изменено с vec3(1.0, 1.0, 1.0)
...
bool Raytrace(SRay ray, float start, float final, inout SIntersection intersect) {
    ...
    for (int i = 0; i < 28; ++i) { // Изменить с 16 на 28
